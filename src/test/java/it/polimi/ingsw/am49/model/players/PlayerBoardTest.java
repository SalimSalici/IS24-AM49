...
class PlayerBoardTest {
    ...
    void placeTile() throws Exception {
        ...
        BoardTile testTile = starterTile.getNeighbourTile(RelativePosition.TOP_RIGHT);
        Assertions.assertEquals(tile1, testTile);
    }

    @Test
    void isPlaceableTile() throws Exception {
        StarterCard start = DeckLoader.getInstance().getNewStarterCardById(81);
        PlayerBoard testBoard = new PlayerBoard(start);
        GoldCard goldTest = DeckLoader.getInstance().getNewGoldCardById(44);
        BoardTile starterTile = testBoard.getStarterTile();
        BoardTile tile1 = testBoard.placeTile(goldTest, starterTile.getRow(), starterTile.getCol(), RelativePosition.TOP_RIGHT);
        boolean test = testBoard.isPlaceableTile(tile1.getRow(), tile1.getCol());
        Assertions.assertFalse(test);
    }

    @Test
    void isCardCostMet() throws Exception {
        //49
        StarterCard start = DeckLoader.getInstance().getNewStarterCardById(81);
        PlayerBoard testBoard = new PlayerBoard(start);
        ...
        GoldCard gold1 = DeckLoader.getInstance().getNewGoldCardById(49);
        boolean test = testBoard.isCardCostMet(gold1);
